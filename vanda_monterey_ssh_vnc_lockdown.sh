...
# 8. SSH authentication is limited to users 'systems' and 'vanda'.
#
...
# without allowing direct connections to VNC, while also restricting SSH access to users 'systems' and 'vanda' limited to SSH key pair authentication.
#
...
# 3. The macOS Monterey installation only has users 'systems' and 'vanda'.
# 4. You will enable SSH ('System Preferences' > 'Sharing' > 'Remote Login') and VNC ('System Preferences' > 'Sharing' > 'Screen Sharing').
# 5. You will configure SSH key pair authentication for both users (this script does not do so, but it does disable SSH password authentication).
...
echo "[x] Limiting SSH authentication to macOS Monterey users systems and vanda..."
echo "AllowUsers systems vanda" >> "$sshd_config_file"
